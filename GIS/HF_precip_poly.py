    ...
    if gPoly in files:
        ...
        for day in rrule.rrule(rrule.DAILY, dtstart=start, until=end):
            ...
            if (yr <= 1991):
                ...
                ras = folder + str(day.year) + "a\\" + "PRISM_NM_" + str(day.year) + day.strftime('%m') + day.strftime(
                    '%d') + ".tif"
                if arcpy.Exists(ras):
                    try:
            ...
            if (yr > 1991):
                ...
                ras = folder + str(day.year) + "\\" + "PRISM_NMHW2Buff_" + str(day.year) + day.strftime(
                    '%m') + day.strftime('%d') + ".tif"
                if arcpy.Exists(ras):
                    try:
                        arcpy.CheckOutExtension("Spatial")
                        mask = "C:\\Recharge_GIS\\nm_gauges.gdb\\nm_wtrshds"
